...
The detector is a ones-sided checksum in the Huang--Abraham family. Let
...
\cite{higham2002accuracy}. We use this for its structural content only.
...
to a GPU. Chai et al.\ argue exactly this, that algorithm-level injection
misses instruction-level and micro-architectural effects and therefore does not
reproduce realistic fault behaviour \cite{chai2026llm}. We concede the point
...
fleet silent-corruption literature \cite{hochschild2021cores,dixit2021silent}
...
Every quantitative claim in this paper was predicted before it was measured.
...
prediction: the detection predicate described above, and a results document
citing a data file under a path that never existed. Both are corrected in
Section~\ref{sec:results:retractions}. Writing the bar down first constrains
...
Code, measurement data, the residual and perturbation specifications, every
pre-registration document with its commit timestamp, and the analysis scripts
are released under Apache 2.0 at
\url{https://github.com/GautamTalksDev/assay-gpu}. Results in this paper
correspond to the tagged release. The pre-registration documents are of more
use than the code: they are how a reader checks that the bars in
Section~\ref{sec:results} predate the numbers they are compared against.
